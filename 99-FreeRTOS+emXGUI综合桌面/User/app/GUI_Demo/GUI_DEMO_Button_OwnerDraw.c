...
//	SetAlpha(hdc,100);
	FillRoundRect(hdc,&rc,8); //用矩形填充背景

//	DrawRoundRect(hdc,&rc,8); //画矩形外框
...
static	LRESULT	win_proc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
  ...
  static uint32_t FPS = 0;
    ...
    hdc_mem = CreateMemoryDC((SURF_FORMAT)SURF_SCREEN, GUI_XSIZE, GUI_YSIZE);
...
//				CreateWindow(BUTTON,L"Button6\r\nOwnerDraw",WS_TRANSPARENT|WS_OWNERDRAW|WS_VISIBLE,rc.x,rc.y,rc.w,rc.h,hwnd,ID_BTN6,NULL,NULL);
...
//      TextOut(hdc,8,4,L"FPS:",-1);
      BitBlt(hdc,  0 ,0,800,  
               480, hdc_mem, 0 , 0, SRCCOPY);
      FPS++;
...
void	GUI_DEMO_Button_OwnerDraw(void)
{
		HWND	hwnd;
		WNDCLASS	wcex;
		MSG msg;
