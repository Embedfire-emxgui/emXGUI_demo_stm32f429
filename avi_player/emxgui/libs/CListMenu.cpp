...
//	WCHAR wstr[64],wbuf[64+12];
	...
	if(flag&OBJ_ACTIVE)
	{
      ...
      InflateRect(&rc,-20,0);
		...
		////
    /* 矩形外框 */
		SetPenColor(hdc,MapRGB(hdc,255,250,250));
////		InflateRect(&rc,-10,0);
      DrawRect(hdc,&rc);
//    
...
//	SYS_kernel_ioctl(KERNEL_IOCTL_GET_SYSTEM_POWER_STATE,&pwr,sizeof(pwr));
	...
	MakeMatrixRect(m_rc,&rc_list,0,20,x_num,y_num);
	if(is_ver_list(hwnd))
	{
	...
	////
//         CreateWindow(BUTTON, L"M", BS_FLAT | BS_NOTIFY | WS_VISIBLE,
//			40, 20, 72, 72, hwnd, 0x3001, NULL, NULL);
//         SetWindowFont(GetDlgItem(hwnd, 0x3001), hFont_SDCARD);
...
//         CreateWindow(BUTTON, L"M", BS_FLAT | BS_NOTIFY | WS_VISIBLE,
//			20, 20, 72, 72, hwnd, 0x3001, NULL, NULL);
//         SetWindowFont(GetDlgItem(hwnd, 0x3001), hFont_SDCARD);
