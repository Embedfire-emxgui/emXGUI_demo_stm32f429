...
static LRESULT	number_input_box_win_proc(HWND hwnd,UINT msg,WPARAM wParam,LPARAM lParam)
{
	...
	switch(msg)
	{
		case WM_CREATE: //窗口被创建时产生该消息，可以在这里做一些初始化的操作
			...
			{
				CreateWindow(BUTTON, str_tbl[i],	WS_VISIBLE|WS_BORDER,m_rc[i].x,m_rc[i].y,m_rc[i].w,m_rc[i].h,hwnd,id_tbl[i],hInst,NULL);
			}
			...
			i=0;
			CreateWindow(BUTTON,	L"退出",	BS_3D|WS_VISIBLE|WS_BORDER,m_rc[i].x,m_rc[i].y,m_rc[i].w,m_rc[i].h,hwnd,eID_EXIT,hInst,NULL);
			i=1;
			CreateWindow(BUTTON,	L"确认",	BS_3D|WS_VISIBLE|WS_BORDER,m_rc[i].x,m_rc[i].y,m_rc[i].w,m_rc[i].h,hwnd,eID_OK,hInst,NULL);
			...
			CreateWindow(TEXTBOX,	pMsgData->str_buf,	TBS_RIGHT|TBS_VCENTER|WS_VISIBLE|WS_BORDER,rc.x,rc.y,rc.w,rc.h,hwnd,eID_TEXTBOX,hInst,NULL);
