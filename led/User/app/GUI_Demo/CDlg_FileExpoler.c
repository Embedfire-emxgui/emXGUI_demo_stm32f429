...
static void scan_file(const char *path,HWND hListWnd)
{
	...
	if(dir!=NULL)
	{
		while(SYS_readdir(dir,&d_info)==SYS_NO_ERR)
		{
			...
			SendMessage(hListWnd,LB_SETITEMDATA,i,(LPARAM)d_info.attr); //通过设置Item项的DATA值来记录文件的属性(用于区分是文件夹和文件).
		...
		case WM_NOTIFY:
		{
			...
			if(id==eID_NEXT) //进入下一级.
			{
				...
				if(i > 0)
				{
					...
					if(i >= 0)
					{
						if(SendMessage(wnd,LB_GETITEMDATA,i,0)&FA_DIR) //如果选中的Item项是文件夹,则进入目录并重新搜索.
						{
							SendMessage(wnd,LB_GETTEXT,i,(LPARAM)wbuf);
							x_wcstombs(buf,wbuf,256);
...
BOOL	CDlg_FileExpoler(HWND hwndParent,int x,int y,int w,int h,const WCHAR *Name,const char *cur_dir,char *out)
{
	...
	ex_data.pszOutDir =(char*)vmalloc(1024);
