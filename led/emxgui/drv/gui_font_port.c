  ...
  {  
    ...
    if(font_base > 0)
    {
      ...
      if(pFontData_XFT!=NULL)
      {
        ...
        hDefaultFont = XFT_CreateFont(pFontData_XFT);
      }
    }
  }
#elif (GUI_USE_EXTERN_FONT)   
  {
    ...
    { 
      ...
      CatalogTypeDef dir;
    	...
    	if(font_base > 0)
    	{
    		hDefaultFont =XFT_CreateFontEx(font_read_data_exFlash,font_base);
    	}
    }
    if(hZoomFont==NULL)
    { 
      ...
      CatalogTypeDef dir;
    	...
    	font_base =RES_GetInfo_AbsAddr(GUI_EXTERN_FONT, &dir);
    	if(font_base > 0)
    	{
    		hZoomFont =XFT_CreateFontEx(font_read_data_exFlash,font_base);
    	}
    }
    ...
    */
    ...
    { 
      /* 从本地加载(本地数组数据) */    	
      hDefaultFont =XFT_CreateFont(GUI_DEFAULT_FONT);  /*ASCii字库,20x20,4BPP抗锯齿*/
